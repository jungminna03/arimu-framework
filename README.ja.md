...
## 追加ユーティリティ — `arimu/Select.hpp`（任意）

ECS から独立した小さなヘッダ（任意）。ゲームプレイ全般で繰り返される「コンテキスト上での選択」パターンを
一箇所にまとめる: **候補（Options）+ コンテキストから事実を読む評価器 + ピッカー**。スキルの入札・ダイアログの
ゲート・クエストの精算の唯一の違いは、事実を読むコンテキストの *スコープ*（エンティティ-ローカル vs
ワールド-グローバル）だけ。そのスコープを一つのキー付きアクセサの背後に隠す。

```cpp
enum class Fact { None, Health, KillsOf, Gold };
struct Ctx { float value(Fact f, const char* arg = nullptr) const; };   // 事実を float に解決

Arimu::Cond<Fact> conds[] = {{Fact::Gold, nullptr, 50}, {Fact::None}};   // ゲート: ゴールド ≥ 50
if (Arimu::gate_met_until(conds, ctx, Fact::None)) buy();

const Opt* best = Arimu::pick(opts, n, ctx,                              // スコア付き argmax
    [](const Opt& o, const Ctx& c){ return score(o, c); }, /*min*/ 0.0f);
```

- `Arimu::SelectionContext<C, Key>` — `c.value(key, arg) -> float` を提供するコンテキストの概念（concept）。
- `Arimu::Cond<Key>{key, arg, min}` + `gate_met` / `gate_met_until` — all-pass のブール値ゲート（センチネル Key は呼び出し側が指定）。
- `Arimu::pick(opts, n, ctx, scoreFn, min, out)` — floor を超える argmax（決定的な strict `>`）、scorer は注入。

ヘッダオンリー · アロケーションなし · コンパイル時（エクスポートシンボルなし、ABI 影響なし）。
`App`/`World`/`System` に触れないので ECS はこれに依存しない — 完全に任意。詳しくは
[`USAGE_FOR_AI.md` §6](USAGE_FOR_AI.md) とヘッダのコメントを参照。
